...
// ===================================================================
// Orden sugerido de los cuartos (2026-09-14)
// ===================================================================
//
// Lo que NO esta aqui: la logica de armar la pregunta, leer la respuesta y
// revisarla. Esa corre en el repo con `node uxp-plugin/pruebas/correr.js`,
// sin Premiere y sin que nadie tenga que prender nada. Aqui se queda solo lo
// que de verdad necesita a Premiere: la red, el disco de UXP y los bins.

// EL RIESGO QUE DECIDE SI ESTE DISENO VIVE AQUI O SE MUDA A CLIPIFY.
//
// Se enumera ANTES de llamar porque en este plugin la documentacion de Adobe
// ya mintio tres veces el mismo dia: la fabrica de efectos devolvio un objeto
// sin metodos, `getParam` necesitaba await aunque la referencia dijera que
// no, y un nombre de parametro era propiedad y no metodo.
//
// Si esta prueba falla, NO se sigue construyendo encima: hay que avisarle a
// Bruno, porque la pestana tendria que mudarse a Clipify -- que es la opcion
// que el descarto por gusto y no por imposibilidad, asi que la decision es
// suya y no nuestra. Ver §8 del spec.
registrarPrueba("¿UXP deja llamar a api.deepseek.com?", async () => {
  ...
  if (!hayFetch) {
    return {
      ok: false,
      detalle: "no existe fetch. Globales parecidas: " + (globales.join(", ") || "ninguna"),
    };
  }

  // La llamada mas barata que prueba el camino completo: red, TLS y que el
  // permiso del manifiesto sirva. Con una llave invalida A PROPOSITO -- un
  // 401 que LLEGA ya contesto la pregunta: salio de la computadora y volvio.
  try {
    ...
    return {
      ok: true,
      detalle:
        "la llamada salio y volvio con HTTP " + r.status +
        " (401 aqui es exito: significa que llego). Globales: " + globales.join(", "),
    };
  } catch (e) {
    return { ok: false, detalle: "fetch existe pero trono: " + e.message };
  }
});

// La llave se guarda y se relee. Se usa una de mentiras y se borra al final:
// esta prueba no debe dejar rastro ni tocar la llave de verdad de Bruno.
registrarPrueba("la llave se guarda en la carpeta del plugin y se relee", async () => {
  const previa = await leerLlave();
  const deMentiras = "sk-prueba-0000-1111-2222";

  try {
    await guardarLlave(deMentiras);
    const releida = await leerLlave();
    const tapada = llaveTapada(releida);

    const ok = releida === deMentiras && tapada.endsWith("2222") && !tapada.includes("prueba");
    return {
      ok: ok,
      detalle: "se releyo igual=" + (releida === deMentiras) + " | tapada=" + tapada,
    };
  } finally {
    // Se devuelve lo que habia. Si Bruno ya tenia su llave puesta, esta
    // prueba no se la puede tumbar.
    if (previa) await guardarLlave(previa);
    else await borrarLlave();
  }
});

// Los cuartos salen de «02. Clip», y «Sin clasificar» no es uno de ellos.
registrarPrueba("cuartosDeLosBins lee los cuartos y se salta «Sin clasificar»", async (project) => {
  const premierepro = require("premierepro");
  const rootItem = await project.getRootItem();
  const rootFolder = premierepro.FolderItem.cast(rootItem);

  // Se arma el arbol de prueba con la misma `resolveBinChain` de produccion.
  await resolveBinChain(project, rootFolder, [CARPETA_DE_CLIPS, "PruebaOrden Fachada"]);
