...
Se comunican por un **manifest JSON** que la app externa escribe en una carpeta de "pendientes"; el plugin la vigila y procesa. No hay red, ni servidor, ni puertos.
...
Un JSON por exportación, con al menos:
...
`ruta_proxy` es `null` si no hay proxy para ese clip. `in_frame`/`out_frame` son `null` si el clip va completo.

## 12. El plugin UXP

- **Instalación:** carpeta copiada directo a `~/Library/Application Support/Adobe/UXP/Plugins/External/<id>_<version>` — no requiere UXP Developer Tools corriendo después de instalado.
- **Vigilancia:** revisa la carpeta de pendientes cada pocos segundos (poll, no push).
- **Por cada manifest encontrado**, y por cada clip dentro de él:
  1. Resuelve/crea la cadena de bins anidados (`categoria_path`).
  2. **Busca primero si el clip ya fue importado antes** (por ruta de archivo — evita duplicar en reexportaciones). Si ya existe: lo mueve al bin correcto y actualiza label/in-out/proxy en vez de reimportar. Si no existe: lo importa dentro del bin final (`project.importFiles`, que preserva la rotación real).
...
- **Feedback visible:** el panel muestra una lista simple de resultados (procesado / con error, por clip) — no solo log de consola, algo que el usuario vea sin abrir herramientas de desarrollador.
- Al terminar un manifest (con o sin errores), lo mueve a una carpeta `procesados/` para no reprocesarlo. Si hubo errores, el nombre del archivo lo indica (ej. sufijo `-con-errores`).
...
- Poblar la secuencia de Premiere — el plugin no crea ni toca secuencias, solo bins.
